...
public struct DrawableContainerFactory<Factory: DrawableFactory> where Factory.Transform == CGAffineTransform {
    typealias Transform = Factory.Transform
    ...
    func drawable(from document: SvgDocument) -> DrawableContainer<Transform, CGRect> {
        drawable(from: document.svg)
    }
}

private extension DrawableContainerFactory {
    func drawable(from tag: SvgTag) -> DrawableContainer<Transform, CGRect> {
        .init(
            viewBox: tag.viewBox,
    ...
    func drawable(with tag: any SVGElement, parentStyle style: SVGStyle) -> Drawable<Transform> {
